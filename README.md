...
Allow users to mention other users, insert hashtags, and tag other entities with this lightweight package. It provides a database-friendly format for easy storing and parsing of tags. The package is highly customizable and can be used with the standard `TextField` widget. Try out the [example app](https://wesselvandam.github.io/taggable/) to see it in action!
...
1. **Saving text with tags**: When a piece of text containing tags needs to be saved, you can use the `textInBackendFormat` getter of the `TagTextEditingController` to get the text in the backend format. Do not use the `text` getter, as the internal text representation is different from both the frontend and backend formats.
   ...
   Additionally, if you only need the text that the user actually entered (with tags rendered in frontend form and no internal markers), the `visibleText` property gives you that value directly. It is cached internally and therefore extremely cheap to access.
